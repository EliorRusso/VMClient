...
void ip_header(unsigned char *buffer, int buflen)
{
	...
	if(ntohs(ip->id) > 60000){
			fprintf(log_txt, "\t|-Identification  Corrrupted  : %d\n", ntohs(ip->id));
	}else{
		fprintf(log_txt, "\t|-Identification : %d\n", ntohs(ip->id));
	}
	if (ip->ttl > 255)
	{
		fprintf(log_txt, "\t|-Time To Live Corrupted	    : %d\n", ip->ttl);
	}
	else
	{
...
void udp_header(unsigned char *buffer, int buflen)
{
	...
	union protocol *b = (union protocol *)(buffer + sizeof(struct iphdr) + sizeof(struct ethhdr) + sizeof(struct udphdr));
	fprintf(log_txt, "%d\n", b->array[0]);
	fprintf(log_txt, "%d\n", b->array[1]);
	fprintf(log_txt, "%d\n", b->array[2]);
	fprintf(log_txt, "%d\n", b->array[3]);
...
{
	struct iphdr *ip = (struct iphdr *)(buffer + sizeof(struct ethhdr));
	struct udphdr *udp1 = (struct udphdr *)(buffer + sizeof(struct iphdr) + sizeof(struct ethhdr));
	if (ntohs(udp1->dest) == 55055)
	{
...
int main()
{
	...
	log_txt = fopen("/home/elior/NodeServer/log.txt", "w");
	if (!log_txt)
	{
